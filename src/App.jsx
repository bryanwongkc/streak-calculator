...
import { Trophy, Swords, Flame, RotateCcw, History, RotateCw, Info, Settings2, CheckCircle2, MessageSquare, UserCircle, XCircle, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 font-sans">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all border ${isEditingNames ? 'bg-blue-900/20 border-blue-500/50 text-blue-400' : 'bg-slate-800 border-slate-700 text-slate-300'}`}
            >
              <UserCircle size={18} /> {isEditingNames ? 'Lock Names' : 'Edit Names'}
            </button>
            <button 
              onClick={() => setShowAdjustments(!showAdjustments)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all border ${showAdjustments ? 'bg-amber-900/20 border-amber-500/50 text-amber-400' : 'bg-slate-800 border-slate-700 text-slate-300'}`}
            >
            ...
            {showResetConfirm ? (
              <div className="flex gap-1 animate-in fade-in zoom-in duration-200">
                <button 
                  onClick={handleReset}
                  className="flex items-center gap-1 px-3 py-2 bg-rose-600 text-white rounded-l-lg font-bold border border-rose-500 shadow-lg shadow-rose-900/20"
                >
                  Confirm Reset
                </button>
                <button 
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-rose-900/40 text-rose-400 rounded-lg transition-all border border-slate-700"
              >
        ...
        {showAdjustments && (
          <div className="bg-amber-950/10 border-2 border-amber-900/30 rounded-2xl p-6 animate-in slide-in-from-top duration-300">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-amber-400 flex items-center gap-2">
                <Settings2 size={20} /> Manual Bank Adjustments
              </h2>
              <div className={`px-3 py-1 rounded-full text-xs font-bold border ${adjSum === 0 ? 'bg-emerald-900/20 border-emerald-500/50 text-emerald-400' : 'bg-rose-900/20 border-rose-500/50 text-rose-400'}`}>
              ...
              {players.map(p => (
                <div key={p.id} className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-amber-400 font-mono focus:ring-2 focus:ring-amber-500 outline-none"
                    placeholder="+/- Points"
                  />
                </div>
              ))}
            ...
            <div className="space-y-2 mb-6">
              ...
              <input 
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-slate-200 outline-none focus:ring-2 focus:ring-amber-500"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {players.map((p) => (
            <div key={p.id} className={`p-5 rounded-2xl border-2 transition-all duration-500 ${lastWinner === p.id ? 'bg-emerald-950/20 border-emerald-500/50 shadow-lg shadow-emerald-500/10' : 'bg-slate-900 border-slate-800'}`}>
              <div className="flex justify-between items-start mb-4">
                <span className="font-bold text-slate-400 text-xs tracking-widest uppercase">{p.id}</span>
                {lastWinner === p.id && <Flame className="text-orange-500 animate-pulse" size={20} />}
              </div>
              
              {isEditingNames ? (
                <input 
                  autoFocus
                  className="bg-slate-800 border border-blue-500/50 rounded px-2 py-1 text-xl font-bold w-full text-white outline-none focus:ring-2 focus:ring-blue-500"
                  value={p.name}
              ...
              <div className="space-y-3 mt-4">
                <div>
                  ...
                  <p className={`text-2xl font-mono ${p.total >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    {p.total.toFixed(1)}
                  </p>
                </div>
                <div className="pt-2 border-t border-slate-800">
                  <p className="text-[10px] uppercase text-slate-500 font-semibold mb-1">Current Tab (Debt)</p>
                  <p className={`text-xl font-mono ${p.debt > 0 ? 'text-amber-400' : 'text-slate-600'}`}>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-xl font-bold text-white">Record New Round</h2>
          </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {players.map(p => (
                  <button
                    ...
                    className={`py-3 px-4 rounded-xl border-2 transition-all flex items-center justify-center gap-2 font-bold ${currentWinner === p.id ? 'bg-blue-600 border-blue-400 text-white shadow-lg shadow-blue-500/20' : 'bg-slate-800 border-transparent text-slate-400 hover:bg-slate-700'}`}
                  >
              ...
              <div className="grid grid-cols-1 gap-3">
                {players.map(p => (
                  <div key={p.id} className={`flex items-center gap-3 ${currentWinner === p.id ? 'opacity-30' : ''}`}>
                    <span className="w-24 text-sm font-semibold truncate">{p.name}:</span>
                    <input 
                      type="number"
          ...
          <div className="mt-8 flex justify-end">
            <button
              ...
              className="px-8 py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-500/20 flex items-center gap-2"
            >
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-300">Match History</h2>
          </div>
          <div className="space-y-3">
            ...
            {history.map((h, i) => (
              <div key={i} className="bg-slate-900/80 border border-slate-800 p-5 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`font-bold text-lg ${h.winner === 'SYSTEM' ? 'text-amber-400' : 'text-emerald-400'}`}>
                ...
                <div className="grid grid-cols-4 gap-4 md:gap-8 bg-slate-950/50 p-3 rounded-lg border border-slate-800/50">
                  {h.scores.map(s => (
                    <div key={s.id} className="flex flex-col items-center min-w-[60px]">
                      ...
                      <span className={`text-sm font-mono font-bold ${s.total >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {s.total > 0 ? `+${s.total.toFixed(0)}` : s.total.toFixed(0)}
                      </span>
                      <span className="text-[9px] text-amber-500/70">
                        {s.debt > 0 ? `Tab: -${s.debt.toFixed(0)}` : ''}
                      </span>
                    </div>
                  ))}
                </div>
        ...
        {/* Legend */}
        <div className="bg-slate-900/30 border border-slate-800/50 rounded-xl p-6 text-xs text-slate-400">
           <div className="flex items-center gap-2 mb-4 text-slate-200 font-bold uppercase tracking-widest text-[10px]">
